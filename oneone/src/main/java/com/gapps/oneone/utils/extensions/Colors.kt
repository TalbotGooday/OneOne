...
@ColorInt
internal fun Int.getPrimaryTextColor(): Int {
	return getPrimaryTextColor(ColorUtils.calculateLuminance(this) > THRESHOLD_LIGHT)
}

@ColorInt
internal fun Int.getPrimaryIconColor(): Int {
...
internal fun Int.isColorLight() = ColorUtils.calculateLuminance(this) > THRESHOLD_LIGHT
...
internal fun View.backgroundTint(@ColorInt primaryTextColor: Int) {
...
internal fun Int.toColorDrawable(@Px size: Int): ColorDrawable {
	return ColorDrawable(this).apply {
		setBounds(0, 0, size, size)
	}
}

internal fun getLogIndicatorByType(type: String): Int {
	return when (type) {
		DEBUG -> {
